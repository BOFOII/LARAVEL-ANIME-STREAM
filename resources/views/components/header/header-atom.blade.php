...
<ul class="header__nav">
    <li class="header__nav-item">
        <a class="header__nav-link" href="{{ route('view.home') }}">{{ __('pages.home_title') }}</a>
    </li>
    <li class="header__nav-item">
        <a class="header__nav-link" href="{{ route('view.ongoing-anime') }}">{{ __('pages.ongoing_title') }}</a>
    </li>
    <li class="header__nav-item">
        <a class="header__nav-link" href="{{ route('view.complete-anime') }}">{{ __('pages.complete_title') }}</a>
    </li>
    <li class="header__nav-item">
        <a class="header__nav-link" href="{{ route('view.jadwal-anime') }}">{{ __('pages.schedule_title') }}</a>
    </li>
    <li class="header__nav-item">
        ...
        <ul class="dropdown-menu header__nav-menu header__nav-menu--scroll" aria-labelledby="dropdownMenu3">
            <li><a href="{{ route('view.genre-list') }}">{{ __('pages.genre_title') }}</a></li>
            <li><a href="https://otakudesu.watch/">Otakudesu</a></li>
            <li><a href="{{ route('view', ['page' => 'kontak']) }}">{{ __('pages.contact_title') }}</a></li>
            <li><a href="{{ route('view', ['page' => 'tentang-kami']) }}">{{ __('pages.btn_about_us') }}</a></li>
            @guest
                <li><a href="{{ route('view.login') }}">{{ __('pages.btn_login') }}</a></li>
                <li><a href="{{ route('view.register') }}">{{ __('pages.btn_register') }}</a></li>
                <li><a href="{{ route('view.forgot-password') }}">{{ __('pages.btn_forgot_password') }}</a></li>
            @else
...
<div class="header__actions">
    <form action="{{ route('view.home') }}" method="GET" class="header__form">
        <input name="search" class="header__form-input" type="text" placeholder="{{ __('pages.placeholder_search') }}">
    ...
    @guest
    <a href="{{ route('view.login') }}" class="header__user">
        <span>{{ __('pages.btn_login') }}</span>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M20,12a1,1,0,0,0-1-1H11.41l2.3-2.29a1,1,0,1,0-1.42-1.42l-4,4a1,1,0,0,0-.21.33,1,1,0,0,0,0,.76,1,1,0,0,0,.21.33l4,4a1,1,0,0,0,1.42,0,1,1,0,0,0,0-1.42L11.41,13H19A1,1,0,0,0,20,12ZM17,2H7A3,3,0,0,0,4,5V19a3,3,0,0,0,3,3H17a3,3,0,0,0,3-3V16a1,1,0,0,0-2,0v3a1,1,0,0,1-1,1H7a1,1,0,0,1-1-1V5A1,1,0,0,1,7,4H17a1,1,0,0,1,1,1V8a1,1,0,0,0,2,0V5A3,3,0,0,0,17,2Z"/></svg>
    </a>
    @else
    <form id="form-logout" class="d-none" action="{{ route('post.logout') }}" method="POST">
        @csrf
    </form>
    <a href="#" onclick="
