        ...
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="row">
                        <div class="col-12">
                            <h2 class="section__title">{{ __('pages.ongoing_title') }}</h2>
                            <p class="section__text">{!! __('pages.ongoing_desc') !!}</p>
                        </div>
                    </div>
                    <div class="row row--grid">
                        @foreach ($anime_list as $anime)
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-2">
                                <div class="card">
                                    <a href="{{ route('view.watch-anime', ['slug' => $anime['id']]) }}"
                                        class="card__cover">
                                    ...
                                    <h3 class="card__title"><a
                                            href="{{ route('view.watch-anime', ['slug' => $anime['id']]) }}">{{ $anime['title'] }}</a>
                                    </h3>
                                    <ul class="card__list">
                                        <li>{{ $anime['episode'] }}</li>
                                        <li>{{ $anime['uploaded_on'] }} - {{ $anime['day_updated'] }}</li>
                                    </ul>
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="row">
                <div class="col-12">
                    <div class="catalog__paginator-wrap">
                        <span class="catalog__pages">{{ __('pages.paginate', [
                            'current' => $page,
                            'end' => end($paginates)
                            ]) }}</span>
                        ...
                        <ul class="catalog__paginator">
                            <li>
                                <a href="{{ route('view.ongoing-anime', ['page' => $page == 1 ? 1 : $page - 1]) }}">
                            ...
                            @foreach ($paginates as $pagination)
                                @if (is_int(intval($pagination)))
                                    <li class="@if ($pagination == $page) active @endif"><a
                                            href="{{ route('view.ongoing-anime', ['page' => $pagination]) }}">{{ $pagination }}</a>
                                    </li>
                                @else
                                @break
                            @endif
                        ...
                        @if ($page != end($paginates))
                            <li>
                                <a href="{{ route('view.ongoing-anime', ['page' => $page + 1]) }}">
                                    <svg width="14" height="11" viewBox="0 0 14 11" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
